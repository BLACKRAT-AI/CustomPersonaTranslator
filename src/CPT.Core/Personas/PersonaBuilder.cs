...
public sealed class PersonaBuilder
{
    ...
    private async Task<string> GenerateSystemPromptAsync(PersonaBuildRequest req, CancellationToken ct)
    {
        ...
        {
            SystemPrompt = "You write concise, faithful system prompts. Output only the prompt."
        };
        var sb = new StringBuilder();
        await foreach (var tok in _llm.StreamRewriteAsync(persona, instr, ct)) sb.Append(tok);
        var result = sb.ToString().Trim();
        return string.IsNullOrWhiteSpace(result) ? meta + "Match the style of the provided samples." : result;
    }
