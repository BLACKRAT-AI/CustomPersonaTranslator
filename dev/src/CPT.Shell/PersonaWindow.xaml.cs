...
public partial class PersonaWindow : Window
{
    ...
    public PersonaWindow(AppServices? services)
    {
        ...
        _services.OnAgentActivity += text => Dispatcher.BeginInvoke(() => { MicSurface.ToolTip = text; MicHint.Text = text; });
        _services.OnAgentsChanged += () => Dispatcher.Invoke(PushActivePersona);
        _services.OnNotification += message => Dispatcher.Invoke(() =>
        {
            Show();
            MicSurface.ToolTip = message;
            MicHint.Text = message;
    ...
    private void OnWebMessage(object? sender, CoreWebView2WebMessageReceivedEventArgs e)
    {
        ...
        try
        {
            ...
            switch (type.GetString())
            {
                ...
                case "avatar_status":
                    if (document.RootElement.TryGetProperty("error", out var error) && error.GetBoolean())
                    { MicHint.Text = document.RootElement.GetProperty("text").GetString(); MicSurface.ToolTip = MicHint.Text; }
                    break;
    ...
    private void ShowAgentBusy(bool busy)
    {
        ...
        SetRingLit(busy);
        if (busy) PostToWeb(new { type = "thinking" });
    }
    ...
    private void PushActivePersona()
    {
        ...
        if (!_agentBusy) MicHint.Text = agent is not null ? "hold to talk" : "set up an agent first";
        ...
        try { PostToWeb(AvatarModelBinding.Message(Web.CoreWebView2, persona.Visual)); }
        catch (Exception ex) { MicHint.Text = ex.Message; MicSurface.ToolTip = ex.Message; }
    }
    ...
    private void SizeToHologram()
    {
        ...
        // Keep the control bar anchored when the avatar canvas changes size.
        Top += Height - height;
        Width = width;
        Height = height;
    }
    ...
    private void ShowStandbyState(StandbyUiState state)
    {
        ...
        if (_agentBusy && state != StandbyUiState.Listening) return;
        ...
        switch (state)
        {
            ...
            case StandbyUiState.Listening:
                ...
                MicHint.Text = "listening — pause when you are done";
                break;
        }
    }

    // Move the visible bar freely; the transparent avatar canvas can extend off-screen.
    private Point? _barDragOrigin;

    private void OnBarDragStart(object sender, MouseButtonEventArgs e)
    {
        if (e.ChangedButton != MouseButton.Left) return;
        _barDragOrigin = e.GetPosition(this);
        Bar.CaptureMouse();
        e.Handled = true;
    }

    private void OnBarDragMove(object sender, MouseEventArgs e)
    {
        if (_barDragOrigin is not { } origin || e.LeftButton != MouseButtonState.Pressed) return;
        var current = e.GetPosition(this);
        Left += current.X - origin.X;
        Top += current.Y - origin.Y;
        e.Handled = true;
    }

    private void OnBarDragEnd(object sender, MouseButtonEventArgs e)
    {
        if (e.ChangedButton != MouseButton.Left || _barDragOrigin is null) return;
        _barDragOrigin = null;
        Bar.ReleaseMouseCapture();
        e.Handled = true;
    }

    private void OnBarDragCaptureLost(object sender, MouseEventArgs e) => _barDragOrigin = null;
    ...
    /// <summary>
    /// Whether a click landed on the stop button, which lives inside the talk
    /// surface.
    ///
    /// PreviewMouseLeftButtonDown TUNNELS: it is raised on the outer element
    /// first, so the talk surface saw every press of stop before the stop
    /// button did, started push-to-talk, and marked the event handled. The
    /// button could not be clicked at all -- pressing it recorded instead. A
    /// child cannot swallow a click its parent is given first; the parent has
    /// to decline it.
    /// </summary>
    private bool StartedOnCancelButton(object? source)
    {
        if (CancelButton.Visibility != Visibility.Visible) return false;

        for (var node = source as DependencyObject; node is not null; node = VisualTreeHelper.GetParent(node))
            if (ReferenceEquals(node, CancelButton)) return true;
    ...
    {
        if (StartedOnCancelButton(e.OriginalSource)) return;
    ...
    private void OnMicUp(object sender, MouseButtonEventArgs e)
    {
        ...
        if (!_micActive || _services is null) return;
        _micActive = false;
        MicSurface.ReleaseMouseCapture();
