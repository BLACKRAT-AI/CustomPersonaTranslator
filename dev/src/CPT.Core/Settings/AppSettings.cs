...
public sealed class StandbySettings
{
    ...
    /// <summary>
    /// Phrases that stop it: they throw away a request being dictated, and cut
    /// short a turn already running or being spoken.
    ///
    /// Several, because the word that comes out when you want something to stop
    /// is whichever one arrives first.
    /// </summary>
    public List<string> CancelPhrases { get; set; } = ["stop", "cancel", "never mind"];

    /// <summary>The single phrase this used to hold, so older settings still load.</summary>
    public string CancelPhrase
    {
        get => CancelPhrases.Count > 0 ? CancelPhrases[0] : "";
        set
        {
            if (string.IsNullOrWhiteSpace(value)) return;
            if (!CancelPhrases.Exists(p => string.Equals(p, value, StringComparison.OrdinalIgnoreCase)))
                CancelPhrases.Insert(0, value.Trim());
        }
    }
...
public sealed class AppSettings
{
    ...
    private void Migrate()
    {
        ...
        foreach (var agent in Agents.Agents) CPT.Core.Agents.AgentSetup.EnsureConfigured(agent);
    }
