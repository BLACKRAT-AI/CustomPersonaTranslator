...
public interface IPersonaRewriter
{
    ...
    /// <summary>
    /// Carries out an instruction and returns what came back.
    ///
    /// Not a rewrite, and that distinction is the whole reason it exists. The
    /// persona builder needs a system prompt WRITTEN; asking for that through
    /// the rewrite path got the request restated instead of carried out, and
    /// the instruction itself -- "produce a SHORT system prompt ... Output the
    /// prompt only" -- was saved as the persona's voice. Every request that
    /// agent then answered arrived with those words attached to it.
    /// </summary>
    Task<string> ComposeAsync(string instruction, CancellationToken ct = default);
}
