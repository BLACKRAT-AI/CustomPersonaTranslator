...
public sealed class LlamaCppClient : IDisposable, IPersonaRewriter
{
    ...
    /// <inheritdoc />
    /// <remarks>
    /// One plain completion, with no persona and no restate rule: the
    /// instruction is the prompt and its result is the answer. Not streamed,
    /// because nothing waits on the first token of a composed prompt.
    /// </remarks>
    public async Task<string> ComposeAsync(string instruction, CancellationToken ct = default)
    {
        var body = JsonSerializer.Serialize(new
        {
            messages = new object[]
            {
                new { role = "system", content = "You carry out the instruction you are given and return only its result." },
