    ...
    [
        new CliProvider
        {
            ...
            Run = new CliRunSpec
            {
                ...
                ContinueArgs =
                    ["-p", "{prompt}", "--resume", "{session}", "--output-format", "stream-json", "--verbose", "{options}"],
        ...
        new CliProvider
        {
            ...
            Run = new CliRunSpec
            {
                ...
                ContinueArgs = ["exec", "resume", "{session}", "--json", "--skip-git-repo-check", "{options}", "{prompt}"],

                // "codex exec" reads a piped prompt on its own, but "codex exec
                // resume" takes [SESSION_ID] [PROMPT] positionally and reads
                // stdin only for a literal "-". Without it every second and
                // later turn resumed the session with nothing to say.
                StdinPromptToken = "-",
            ...
            [
                ...
                new CliOption
                {
                    Id = "speed", Label = "Fast mode",
                    Hint = "Faster model responses on supported models. Uses more credits (Astra: 2.5x). Applies independently to agent and persona settings.",
                    DefaultChoiceId = "default",
                    Choices =
                    [
                        Choice("default", "Use Codex setting"),
                        Choice("fast", "Fast", "-c", "service_tier=\"fast\"", "-c", "features.fast_mode=true"),
                        Choice("standard", "Standard", "-c", "service_tier=\"default\""),
                    ],
                },
                new CliOption
                {
                    Id = "desktop", Label = "Screen, mouse and keyboard",
                    Hint = "Lets the agent see and operate your Windows desktop. Stop the agent to stop desktop control. Applies to the answering agent only.",
                    DefaultChoiceId = "off",
                    Choices = [Choice("off", "Off"), Choice("on", "Enabled (automatic review)")],
                },
                // Sandbox as a config override, not "-s".
                //
                // "codex exec" takes "-s"; "codex exec resume" does not, and
                // rejects it outright -- "error: unexpected argument '-s' found",
                // exit code 2. Since a conversation resumes from its second turn
                // onward, an agent with any approval setting but the default
                // answered once and then failed every time after. "-c
                // sandbox_mode=..." is the same setting and is accepted by both.
                Permissions(
                    Choice("default", "Ask before acting"),
                    Choice("read-only", "Read only", "-c", "sandbox_mode=\"read-only\""),
                    Choice("workspace-write", "Write in workspace", "-c", "sandbox_mode=\"workspace-write\""),
                    Choice("danger-full-access", "Full filesystem and network access",
