...
export class HoloHead {
  constructor(url, framing) {
    this.ok = true;
    this.loaded = false;
    this.targets = [];
    this.mouthN = [0, 0.12];
    if (url) this._loadModel(url, framing);
    else this._load();
  }

  async _loadModel(url, framing) {
    try {
      const { loadModelHead } = await import('./model_head.js');
      const result = await loadModelHead(url, framing);
      if (this.cancelled) return;
      this.targets = result.targets;
      this.mouthN = result.mouth;
      this.loaded = true;
      window.dispatchEvent(new CustomEvent('avatar-status', {detail: {text: result.status, error: false}}));
    } catch (error) {
      if (this.cancelled) return;
      this.ok = false;
      window.dispatchEvent(new CustomEvent('avatar-status', {detail: {text: 'Could not load 3D avatar: ' + error.message, error: true}}));
    }
  }
  ...
  async _load() {
    try {
      ...
      this.loaded = true;
      window.dispatchEvent(new CustomEvent('avatar-status', {detail: {text: 'Default head loaded', error: false}}));
    } catch {
