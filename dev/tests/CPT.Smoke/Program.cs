...
if (args.Length > 0 && args[0] is "verify-jarvis" or "verify-persona-voice")
{
    var persona = new PersonaStore().Get(args.Length > 1 ? args[1] : "jarvis") ?? throw new InvalidOperationException("Jarvis is missing");
    using var clone = new ChatterboxTts(settings.ChatterboxPython, settings.ChatterboxScript);
    clone.Expressiveness = persona.Voice.Expressiveness;
    clone.Progress = new Progress<string>(message => { if (message.Contains("reference", StringComparison.Ordinal)) Console.WriteLine(message); });
    using var timeout = new CancellationTokenSource(TimeSpan.FromSeconds(150));
    var timer = System.Diagnostics.Stopwatch.StartNew();
    var bytes = new System.Collections.Generic.List<byte>();
    Console.WriteLine("Synthesizing with " + persona.Name + " sample: " + persona.Voice.VoiceSampleFile);
    await foreach (var chunk in clone.SynthesizeStreamAsync(persona.Id == "jarvis" ? "At your service, sir. I am using your selected voice." : persona.Id == "c3po" ? "Oh, my! I am C-3PO, human-cyborg relations." : "Acknowledged. Computer voice interface is operational.", persona.Voice.VoiceSampleFile!, timeout.Token)) bytes.AddRange(chunk);
    if (bytes.Count < 4800 || !bytes.Any(b => b != 0)) throw new InvalidOperationException("No audible PCM generated");
    var path = Path.Combine(Path.GetTempPath(), "CPT-" + persona.Id + "-voice-check.wav");
    using (var writer = new NAudio.Wave.WaveFileWriter(path, new NAudio.Wave.WaveFormat(clone.SampleRate, clone.BitsPerSample, clone.Channels))) writer.Write(bytes.ToArray(), 0, bytes.Count);
    Console.WriteLine($"PASS actual clone synthesis: {bytes.Count} PCM bytes, {timer.Elapsed.TotalSeconds:F1}s, {path}");
    return;
}

if (args.Length > 0 && args[0] == "verify-voice")
{
    await CPT.Smoke.VoiceProtocolVerification.RunAsync(settings);
    return;
}

if (args.Length > 0 && args[0] == "verify-agent")
{
    await CPT.Smoke.AgentVerification.RunAsync(settings);
    return;
}
...
if (args.Length >= 1 && args[0] == "sayit")
{
    ...
    foreach (var sentence in sentences)
    {
        ...
        Console.WriteLine($"[say]   \"{sentence}\" -> {bytes} bytes in {one.ElapsedMilliseconds} ms");
        spokenBytes += bytes;
    }
    (engine as IDisposable)?.Dispose();
...
if (args.Length >= 1 && args[0] == "clonecancel")
{
    // One press of Cancel used to silence the voice for the rest of the
    // session: the request had already been written, its answer stayed in the
    // pipe, and every later synthesis read THAT answer and went looking for a
    // file the server had not written -- "could not find file
    // cpt_clone_....wav", for ever. This drives exactly that sequence.
    if (!ChatterboxTts.IsAvailable(settings.ChatterboxPython, settings.ChatterboxScript))
    {
        Console.WriteLine("[clone] voice cloning is not set up on this machine; nothing to check.");
        return;
    }
    ...
    long first = 0;
    await foreach (var chunk in clone.SynthesizeStreamAsync("One.", reference)) first += chunk.Length;
    Console.WriteLine($"[clone] before  : {first} bytes");

    using (var giveUp = new CancellationTokenSource(TimeSpan.FromMilliseconds(150)))
    {
        try
        {
            await foreach (var _ in clone.SynthesizeStreamAsync("Two, abandoned.", reference, giveUp.Token)) { }
            Console.WriteLine("[clone] cancel  : finished before the cancel landed");
        }
        catch (OperationCanceledException) { Console.WriteLine("[clone] cancel  : abandoned mid-synthesis"); }
    }

    long after = 0;
    await foreach (var chunk in clone.SynthesizeStreamAsync("Three.", reference)) after += chunk.Length;

    Console.WriteLine(after > 0
        ? $"[clone] after   : {after} bytes — the voice survived the cancel."
        : "[clone] after   : NO AUDIO — the cancel silenced it. This is the bug.");
    return;
}
...
if (args.Length >= 1 && args[0] == "hearcheck")
{
    ...
    var hint = string.Join(", ", new[] { settings.Standby.WakePhrase }
...
if (args.Length >= 1 && args[0] == "standby")
{
    ...
    Console.WriteLine("[standby] stop   = \"" + string.Join("\", \"", settings.Standby.CancelPhrases) + "\"");
    ...
    listener.StopRequested += () => Console.WriteLine("[standby] STOP");
    ...
    Console.WriteLine($"[standby] listening for {listenFor}s — say the wake phrase, then a question, then pause.");
