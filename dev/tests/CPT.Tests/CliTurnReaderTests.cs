...
public class CliTurnReaderTests
{
    ...
    [InlineData("null")]
    [InlineData("\"pending\"")]
    public void Codex_nullable_exit_code_does_not_abort_the_turn(string exitCode)
    {
        var events = Run(new CodexJsonLinesReader(),
            "{\"type\":\"item.started\",\"item\":{\"type\":\"command_execution\",\"exit_code\":" + exitCode + "}}",
            """{"type":"item.completed","item":{"type":"agent_message","text":"Checked the result."}}""");
        Assert.Equal("Checked the result.", AssistantText(events));
        Assert.Contains(events, e => e.Kind == CliTurnEventKind.Activity);
    }
...
/// <summary>
/// What the user HEARS while a long task runs.
///
/// Codex narrates its way through one: "FL Studio is open, sir; I am preparing
/// the arrangement". Those messages used to be filed with the tool chatter and
/// dropped, so a three-minute task was three minutes of silence and then one
/// sentence -- which from across the room is an app that has stopped working.
/// </summary>
public class CodexProgressTests
{
    private static List<CliTurnEvent> Drive(params string[] lines)
    {
    ...
    [Fact]
    public void Every_agent_message_is_offered_as_progress_while_the_turn_runs()
    {
        var events = Drive(Message("Checking the studio, sir."), Message("Preparing the arrangement, sir."));

        var progress = events.Where(e => e.Kind == CliTurnEventKind.Progress).Select(e => e.Text).ToList();
        Assert.Equal(["Checking the studio, sir.", "Preparing the arrangement, sir."], progress);
    }

    [Fact]
    public void The_answer_is_the_last_message_and_is_only_delivered_once()
    {
        var events = Drive(Message("Checking the studio, sir."), Message("Playback has started, sir."));

        var answer = events.Where(e => e.Kind == CliTurnEventKind.AssistantText).Select(e => e.Text).ToList();
        Assert.Equal(["Playback has started, sir."], answer);
    }

    [Fact]
    public void Tool_chatter_is_still_never_spoken()
    {
        var events = Drive("{\"type\":\"item.completed\",\"item\":{\"item_type\":\"command_execution\",\"text\":\"powershell -Command ...\"}}");

        Assert.DoesNotContain(events, e => e.Kind == CliTurnEventKind.Progress);
        Assert.DoesNotContain(events, e => e.Kind == CliTurnEventKind.AssistantText);
    }
}
