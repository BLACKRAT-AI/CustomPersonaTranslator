...
All inference — the LLM rewrite, text-to-speech and speech recognition — runs on
your machine. Nothing leaves it except the coding CLI's own traffic to whichever
vendor you signed in to, plus optional Wikipedia/Wikiquote lookups when you turn
on Research Mode while building a persona.
...
3. Say the **send phrase** — "send it" by default — and it goes to the agent.

"never mind" throws the request away, and a configurable silence timeout sends
what you dictated if you forget the send phrase. All three phrases are yours to
change under Settings → Standby, along with microphone sensitivity.
...
- A local `.gguf` chat model served by llama.cpp, for the persona rewrite
